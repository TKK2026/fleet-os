...
export default function DetailPanel({ vehicle: v, settings, onClose, onUpdateAA }: Props) {
    ...
    <div className="bg-white border border-[#E4E2DC] rounded-xl p-5 shadow-sm mb-4">
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <div className="text-xs text-[#5A5850] mt-0.5">
            {v.owner} ｜ 取得日 {v.acquired} ｜ {v.color} ｜ VIN: {v.vin}
          </div>
        </div>
        <button onClick={onClose} className="text-[#9A9890] hover:text-[#1A1916] transition-colors text-xl leading-none">
          <X size={18} />
        </button>
      </div>
      ...
      {/* alert */}
      {!c.aa ? (
        <div className="mb-4 p-3 rounded-lg border-l-4 border-[#378ADD] bg-[#E6F1FB] text-[#0C447C] text-xs">
          AA相場が未入力です。右側の入力欄から査定額を入力すると売却損益・税引後手取りがリアルタイムで計算されます。
        </div>
      ) : c.gain !== null && c.gain < 0 ? (
        <div className="mb-4 p-3 rounded-lg border-l-4 border-[#E24B4A] bg-[#FCEBEB] text-[#791F1F] text-xs">
          📉 売却損 {yenM(Math.abs(c.gain))} — AA相場が簿価＋補助金を下回っています。売却タイミングを再検討するか相場回復を待つことを推奨します。
        </div>
      ) : c.gain !== null && c.gain > 0 ? (
        <div className="mb-4 p-3 rounded-lg border-l-4 border-[#639922] bg-[#EAF3DE] text-[#27500A] text-xs">
          ✅ 売却益 {yenM(c.gain)} — 法人税負担 {yenM(c.corp_tax_amount)} を考慮した税引後手取りは {yenM(c.take_home)} です。
        </div>
      ) : null}
            ...
            {c.aa ? (
              <div className="space-y-0">
                {[
                  ...
                  { k: `消費税額（買手負担・参考 ${settings.consumption_tax}%）`, v: c.ct_amount ? yen(c.ct_amount) : '−' },
        ...
        <div className="bg-[#F2F0EC] border border-[#E4E2DC] rounded-lg p-4">
          ...
          <div className="flex gap-2 mb-1">
            <input
              ...
              step={10000}
              className="flex-1 text-sm px-3 py-2 rounded-lg border border-[#CFCDC6] bg-white focus:outline-none focus:border-[#0C447C]"
            />
            <button
              onClick={handleApply}
              className="px-4 py-2 bg-[#1A1916] text-white text-xs rounded-lg hover:opacity-80 transition-opacity font-medium"
            >
              更新
            </button>
          </div>
          <div className="flex items-center gap-1 text-[10px] text-[#9A9890] mb-5">
            <Zap size={10} />
          ...
          {c.aa && (
            <>
              <div className="text-[10px] font-semibold tracking-widest uppercase text-[#5A5850] mb-3 mt-2">税引後 手取りサマリー</div>
